import {ImpostureLang} from './types';
...
function doFail(streamState: JSONStreamState, msg: string): void {
  ...
  throw new Error(
    'Near offset ' + streamState.pos + ': ' + msg + ' ~~~' + streamState.source.substr(streamState.pos, 50) + '~~~'
  );
}
  ...
  while (nextJSONToken(streamState, token)) {
    if (state === JSONState.ROOT_STATE) {
      ...
      if (token.type === JSONTokenType.LEFT_CURLY_BRACKET) {
        cur = {};
        if (withMetadata) {
          cur.$impostureLang = token.toLocation(filename);
        }
        pushState();
    ...
    if (state === JSONState.DICT_STATE || state === JSONState.DICT_STATE_NO_CLOSE) {
      ...
      if (token.type === JSONTokenType.STRING) {
        ...
        if (token.type === JSONTokenType.LEFT_CURLY_BRACKET) {
          const newDict: any = {};
          if (withMetadata) {
            newDict.$impostureLang = token.toLocation(filename);
          }
...
class JSONToken {
  ...
  /**
   * Reuse ImpostureLang as the location field among parsed json segments
   * @param optional filename
   */
  toLocation(filename?: string): ImpostureLang {
    return {
      filename: filename,
      line: this.line,
      char: this.char,
