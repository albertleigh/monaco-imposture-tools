...
import { OnigString, OnigScanner, IOnigCaptureIndex } from '@monaco-imposture-tools/oniguruma-asm';
...
export abstract class Rule {
	...
	public get debugName(): string {
		return `${(<any>this.constructor).name}#${this.id} @ ${this.$impostureLang?.filename}:${this.$impostureLang?.line}`;
	}
	 ...
	 * it would be collected as an entry and only check the condition of the rule
	 *
	 */
...
export class CaptureRule extends Rule {
	constructor(
		$impostureLang: ImpostureLang, id: number, name: string, contentName: string,
...
export function createOnigString(sources: string): OnigString {
	const r = new OnigString(sources);
	(r as any).$str = sources;
	return r;
}
...
export class IncludeOnlyRule extends Rule {
	...
	constructor($impostureLang: ImpostureLang, id: number, name: string, contentName: string, patterns: ICompilePatternsResult) {
		super($impostureLang, id, name, contentName);
...
export class BeginEndRule extends Rule {
	...
	constructor(
		$impostureLang: ImpostureLang, id: number, name: string, contentName: string,
		begin: string, beginCaptures: CaptureRule[],
		end: string, endCaptures: CaptureRule[],
...
export class BeginWhileRule extends Rule {
	...
	constructor($impostureLang: ImpostureLang, id: number, name: string, contentName: string, begin: string, beginCaptures: CaptureRule[], _while: string, whileCaptures: CaptureRule[], patterns: ICompilePatternsResult) {
		super($impostureLang, id, name, contentName);
		this._begin = new RegExpSource(begin, this.id);
		this.beginCaptures = beginCaptures;
...
export class RuleFactory {
	...
	public static createCaptureRule(helper: IRuleFactoryHelper, $impostureLang: ImpostureLang, name: string, contentName: string, retokenizeCapturedWithRuleId: number): CaptureRule {
	...
	private static _compileCaptures(captures: IRawCaptures, helper: IRuleFactoryHelper, repository: IRawRepository): CaptureRule[] {
		...
		if (captures) {
			...
			for (captureId in captures) {
				...
				r[numericCaptureId] = RuleFactory.createCaptureRule(helper, captures[captureId].$impostureLang, captures[captureId].name, captures[captureId].contentName, retokenizeCapturedWithRuleId);
			}
		}
