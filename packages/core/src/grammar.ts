...
import {
	...
	MatchRuleASTNode,
	IRawGrammar, IRawRepository, IRawRule, Position, Range
} from './types';
...
export class Grammar implements IGrammar, IRuleFactoryHelper {
	...
	constructor(
		grammar: IRawGrammar, initialLanguage: number, embeddedLanguages: IEmbeddedLanguagesMap,
		tokenTypes: ITokenTypeMap, grammarRepository: IGrammarRepository & IThemeProvider
	) {
	...
	public parse(text:string, option?:{
		separator?: string
	}):CodeDocument{
		option = option || {
			separator: SEPARATOR
		...
		let rootRule = this.getRule(this._rootId);
		let rootScopeName = rootRule.getName(null, null);
		let rawRootMetadata = this._scopeMetadataProvider.getMetadataForScope(rootScopeName);
		let rootMetadata = ScopeListElement.mergeMetadata(defaultMetadata, null, rawRootMetadata);
		...
		const codeDocuments = new CodeDocument(
			text,
			option.separator,
			rootNode
		);
		...
		codeDocuments.lines.forEach((value, index) => {
			// todo acc lineText
			const lineText = value + option.separator;
			const onigText = new OnigString(lineText);
			const parsedResult = _parseString(this, onigText, index === 0, 0, codeDocuments.accLineLen(index), prevState, workingNode);
		...
		codeDocuments.traverse(_oneNode => {
			...
			if (
				typeof oneNode.length === 'number' ||
				typeof oneNode.offset === 'number'
			){
...
function handleCaptures(grammar: Grammar, lineText: OnigString, isFirstLine: boolean, stack: StackElement, lineTokens: LineTokens, captures: CaptureRule[], captureIndices: IOnigCaptureIndex[]): void {
	...
	for (let i = 0; i < len; i++) {
		...
		if (captureRule.retokenizeCapturedWithRuleId) {
			...
			let scopeName = captureRule.getName(lineText.toString(), captureIndices);
			let nameScopesList = stack.contentNameScopesList.push(grammar, scopeName);
			let contentName = captureRule.getContentName(lineText.toString(), captureIndices);
			...
			_tokenizeString(grammar,
				new OnigString(
					lineText.substring(0, captureIndex.end)
				),
		...
		let captureRuleScopeName = captureRule.getName(lineText.toString(), captureIndices);
...
function _tokenizeString(grammar: Grammar, lineText: OnigString, isFirstLine: boolean, linePos: number, stack: StackElement, lineTokens: LineTokens): StackElement {
	...
	function scanNext(): void {
		...
		if (matchedRuleId === -1) {
			...
			let scopeName = _rule.getName(lineText.toString(), captureIndices);
			let nameScopesList = stack.contentNameScopesList.push(grammar, scopeName);
			// todo why are name scope and name content scope the same
			...
			if (_rule instanceof BeginEndRule) {
				...
				let contentName = pushedRule.getContentName(lineText.toString(), captureIndices);
				let contentNameScopesList = nameScopesList.push(grammar, contentName);
...
function _parseWhileConditions(
	grammar: Grammar, lineText: OnigString, isFirstLine: boolean, linePos: number, lineOffset:number, stack: StackElement, parent:ASTNode
):IWhileCheckResult{
	let anchorPosition = -1;
	let whileRules: IWhileStack[] = [];
	for (let node = stack; node; node = node.pop()) {
		...
		if (r) {
			...
			if (r.captureIndices && r.captureIndices.length) {
				...
				beginWhileNode.whileCaptureChildren = _parseCaptures(
					grammar, lineText, isFirstLine, lineOffset, whileRule.stack, beginWhileNode,  whileRule.rule.whileCaptures, r.captureIndices
...
function _parseCaptures(
	grammar: Grammar, text: OnigString, isFirstLine: boolean, lineOffset:number, stack: StackElement, parent:ASTNode, captures: CaptureRule[], captureIndices: IOnigCaptureIndex[]
):ASTNode[]{
	if (captures.length ===0){
		return []
	}
	...
	for (let i = 0; i < len; i++){
		...
		if (captureRule.retokenizeCapturedWithRuleId) {
			...
			_parseString(grammar, new OnigString(text.substring(0, captureIndex.end)),
				isFirstLine, captureIndex.start, lineOffset, stackClone, theOne);
			result.push(theOne);
			continue;
		}

		let captureRuleScopeName = captureRule.getName(text.toString(), captureIndices);
		if (captureRuleScopeName !== null) {
			// push
			result.push({
				$impostureLang: captureRule.$impostureLang,
	...
	function doParse() {
		...
		if (matchedRuleId === -1) {
			...
			workingNode.endCaptureChildren = _parseCaptures(
				grammar, lineText, isFirstLine, lineOffset, stack, workingNode, poppedRule.endCaptures, captureIndices
			)
			...
			if (!hasAdvanced && popped.getEnterPos() === linePos) {
				// Grammar pushed & popped a rule without advancing
				console.error('[1.2] - Grammar is in an endless loop - Grammar pushed & popped a rule without advancing');
			...
			let _rule = grammar.getRule(matchedRuleId);
			if (workingNode.children?.length && typeof workingNode.children[workingNode.children.length-1].length !== 'number'){
			...
			if (_rule instanceof BeginEndRule) {
				...
				beginEndNode.beginCaptureChildren = _parseCaptures(
					grammar, lineText, isFirstLine, lineOffset, stack, beginEndNode, pushedRule.beginCaptures, captureIndices
				)
				...
				beginWhileNode.beginCaptureChildren = _parseCaptures(
					grammar, lineText, isFirstLine, lineOffset, stack, beginWhileNode, pushedRule.beginCaptures, captureIndices
				)
				_ensureMutableASTNodeChildren(workingNode);
				...
				matchingNode.children = _parseCaptures(
					grammar, lineText, isFirstLine, lineOffset, stack, matchingNode, matchingRule.captures, captureIndices);
				anchorPosition = captureIndices[0].end;
				...
				if (!hasAdvanced){
					console.error('Parser [4] - Grammar is in an endless loop - Grammar is not advancing, nor is it pushing/popping');
					stack = stack.safePop();
...
export class CodeDocument{
	...
	getAheadNodeByOffset(offset:number):ASTNode | undefined{
		...
		while (workingNode && !found){
			...
			workingNode.children.forEach(one=>{
				if (
					one.offset <= offset &&
					(one.offset+one.length) > offset
				){
					if (!matchingChildNode){
						matchingChildNode = one;
					}else{
						matchingChildNode = matchingChildNode.offset < one.offset ?
							one:
							matchingChildNode
					}
				}
	...
	positionAt(offset:number): Position{
		...
		let theLine = 0, theChar = 0, curOffset=0;
	...
	offsetAt(pos: Position): number{
		...
		if (
			pos.line< 0 ||
			pos.line > this.lines.length
		){
			throw new CodeDocument.POS_NOT_IN_RANGE(pos, this.text?.length||0);
		}else if(
			pos.character < 0 ||
			pos.character > this.lines[pos.line].length
		){
	...
	printASTNode(pretty?:boolean){
		...
		return JSON.stringify(this.root, (key, value) => {
			if (
				key === 'parent' ||
				key.indexOf("$impostureLangNode") > -1
			){
				return undefined;
			}
			return value;
