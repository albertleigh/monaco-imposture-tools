...
import {
	IRawGrammar, RegistryOptions, IRawTheme, IGrammarConfiguration, IEmbeddedLanguagesMap, ITokenTypeMap,
	IGrammar, StackElement,
} from './types';
...
export class Registry {
	...
	constructor(
		private readonly _locator: RegistryOptions = DEFAULT_OPTIONS
	) {
		...
		const prom = new Promise<IGrammar>(async (resolve, reject) => {
			...
			const rawGrammar: IRawGrammar = grammarDefinition.format === 'json'
				? typeof grammarDefinition.content === 'string'
					? parseJSONGrammar(grammarDefinition.content, `/dummy/file/${initialScopeName}`)
